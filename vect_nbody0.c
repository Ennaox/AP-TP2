...
vector vect_add_vectors(vector a, vector b)
{
  vector c;
...
vector vect_scale_vector(double b, vector a)
{
  __asm__ volatile (
...
vector vect_sub_vectors(vector a, vector b)
{
   vector c;
...
{ 
  for (int i = 0; i < nbodies; i++)
	...
	if(i != j)
	  accelerations[i] = vect_add_vectors(accelerations[i],
					 vect_scale_vector(GravConstant * masses[j] / (pow(vect_mod(vect_sub_vectors(positions[i], positions[j])), 3) + 1e7),
						      vect_sub_vectors(positions[j], positions[i])));
    }
}
...
{    
  for (int i = 0; i < nbodies; i++)
    velocities[i] = vect_add_vectors(velocities[i], accelerations[i]);
}
...
{ 
  for (int i = 0; i < nbodies; i++)
    positions[i] = vect_add_vectors(positions[i], vect_add_vectors(velocities[i], vect_scale_vector(0.5, accelerations[i])));
}
...
unsigned test()
{
  ...
  vector d = vect_add_vectors(a,b);

  if(c.x != d.x || c.y != d.y)
    return printf("c.x = %lf, d.x = %lf\nc.y = %lf, d.y = %lf\nERROR vect_add_vectors != vect_add_vectors\n",c.x,d.x,c.y,d.y), 1;

  c = sub_vectors(a,b);
  d = vect_sub_vectors(a,b);

  if(c.x != d.x || c.y != d.y)
    return printf("c.x = %lf, d.x = %lf\nc.y = %lf, d.y = %lf\nERROR vect_sub_vectors != sub_vectors\n",c.x,d.x,c.y,d.y), 1;

  double factor = randreal();
  c = scale_vector(factor,a);
  d = vect_scale_vector(factor,a);

  if(c.x != d.x || c.y != d.y)
    return printf("c.x = %lf, d.x = %lf\nc.y = %lf, d.y = %lf\nERROR vect_scale_vectors != scale_vectors\n",c.x,d.x,c.y,d.y), 1;

  double d1 = mod(a);
  double d2 = vect_mod(a);
